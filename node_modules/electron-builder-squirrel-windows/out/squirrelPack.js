...
exports.SquirrelBuilder = undefined;
...
let pack = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (options, directory, updateFile, outFile, version, packager) {
...
<package xmlns="http://schemas.microsoft.com/packaging/2011/08/nuspec.xsd">
  <metadata>
    ...
    <copyright>${copyright}</copyright>${options.extraMetadataSpecs || ""}
        ...
        (0, (_builderUtil || _load_builderUtil()).debug)(`Created NuSpec file:\n${nuspecContent}`);
...
let msi = (() => {
    var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (options, nupkgPath, setupPath, outputDirectory, outFile) {
        const args = ["--createMsi", nupkgPath, "--bootstrapperExe", setupPath];
        yield (0, (_builderUtil || _load_builderUtil()).exec)(process.platform === "win32" ? _path.join(options.vendorPath, "Update.com") : "mono", (0, (_builderUtil || _load_builderUtil()).prepareWindowsExecutableArgs)(args, _path.join(options.vendorPath, "Update-Mono.exe")));
        //noinspection SpellCheckingInspection
        yield (0, (_builderUtil || _load_builderUtil()).exec)(_path.join(options.vendorPath, "candle.exe"), ["-nologo", "-ext", "WixNetFxExtension", "-out", "Setup.wixobj", "Setup.wxs"], {
            cwd: outputDirectory
        });
        //noinspection SpellCheckingInspection
        yield (0, (_builderUtil || _load_builderUtil()).exec)(_path.join(options.vendorPath, "light.exe"), ["-ext", "WixNetFxExtension", "-sval", "-out", outFile, "Setup.wixobj"], {
            cwd: outputDirectory
        });
        //noinspection SpellCheckingInspection
        yield (_bluebirdLst2 || _load_bluebirdLst2()).default.all([(0, (_fsExtraP || _load_fsExtraP()).unlink)(_path.join(outputDirectory, "Setup.wxs")), (0, (_fsExtraP || _load_fsExtraP()).unlink)(_path.join(outputDirectory, "Setup.wixobj")), (0, (_fsExtraP || _load_fsExtraP()).unlink)(_path.join(outputDirectory, outFile.replace(".msi", ".wixpdb"))).catch(function (e) {
            return (0, (_builderUtil || _load_builderUtil()).debug)(e.toString());
        })]);
    });
...
let encodedZip = (() => {
    var _ref3 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (archive, dir, prefix, vendorPath, packager) {
        yield (0, (_fs || _load_fs()).walk)(dir, null, {
            isIncludeDir: true,
            consume: (() => {
                var _ref4 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (file, stats) {
                    ...
                    if (file.endsWith(".exe") && !file.includes("squirrel.exe") && !relativeSafeFilePath.includes("/")) {
                        ...
                        yield (0, (_builderUtil || _load_builderUtil()).execWine)(_path.join(vendorPath, "WriteZipToSetup.exe"), ["--copy-stub-resources", file, tempFile]);
...
function _load_archive() {
    return _archive = require("electron-builder-lib/out/targets/archive");
}
...
function syncReleases(outputDirectory, options) {
    (0, (_builderUtil || _load_builderUtil()).log)("Sync releases to build delta package");
    const args = (0, (_builderUtil || _load_builderUtil()).prepareWindowsExecutableArgs)(["-u", options.remoteReleases, "-r", outputDirectory], _path.join(options.vendorPath, "SyncReleases.exe"));
    if (options.remoteToken) {
        args.push("-t", options.remoteToken);
    }
    return (0, (_builderUtil || _load_builderUtil()).spawn)(process.platform === "win32" ? _path.join(options.vendorPath, "SyncReleases.exe") : "mono", args);
}
class SquirrelBuilder {
    ...
    buildInstaller(outFileNames, appOutDir, outDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const dirToArchive = yield packager.info.tempDirManager.createTempDir({ prefix: "squirrel-windows" });
            const outputDirectory = _this.outputDirectory;
            const options = _this.options;
            const appUpdate = _path.join(dirToArchive, "Update.exe");
            ...
            const nupkgPath = _path.join(outputDirectory, outFileNames.packageFile);
            const setupPath = _path.join(outputDirectory, outFileNames.setupFile);
            yield (_bluebirdLst2 || _load_bluebirdLst2()).default.all([pack(options, appOutDir, appUpdate, nupkgPath, version, packager), (0, (_fs || _load_fs()).copyFile)(_path.join(options.vendorPath, "Setup.exe"), setupPath), (0, (_fs || _load_fs()).copyFile)(options.loadingGif ? _path.resolve(packager.projectDir, options.loadingGif) : _path.join(options.vendorPath, "install-spinner.gif"), _path.join(dirToArchive, "background.gif"))]);
            ...
            yield _this.releasify(nupkgPath, outFileNames.packageFile).then(function (it) {
                return (0, (_fsExtraP || _load_fsExtraP()).writeFile)(_path.join(dirToArchive, "RELEASES"), it);
            });
            const embeddedArchiveFile = yield _this.createEmbeddedArchiveFile(nupkgPath, dirToArchive);
            yield (0, (_builderUtil || _load_builderUtil()).execWine)(_path.join(options.vendorPath, "WriteZipToSetup.exe"), [setupPath, embeddedArchiveFile]);
    ...
    releasify(nupkgPath, packageName) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const out = (yield (0, (_builderUtil || _load_builderUtil()).exec)(process.platform === "win32" ? _path.join(_this2.options.vendorPath, "Update.com") : "mono", (0, (_builderUtil || _load_builderUtil()).prepareWindowsExecutableArgs)(args, _path.join(_this2.options.vendorPath, "Update-Mono.exe")))).trim();
            if ((_builderUtil || _load_builderUtil()).debug.enabled) {
                (0, (_builderUtil || _load_builderUtil()).debug)(`Squirrel output: ${out}`);
            }
    ...
    createEmbeddedArchiveFile(nupkgPath, dirToArchive) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            yield (0, (_builderUtil || _load_builderUtil()).exec)((_zipBin || _load_zipBin()).path7za, (0, (_archive || _load_archive()).compute7zCompressArgs)("zip", _this3.packager.compression, { isRegularFile: true }).concat(embeddedArchiveFile, "."), {
                cwd: dirToArchive
            });
            yield (0, (_builderUtil || _load_builderUtil()).exec)((_zipBin || _load_zipBin()).path7za, (0, (_archive || _load_archive()).compute7zCompressArgs)("zip", "store" /* nupkg is already compressed */, { isRegularFile: true }).concat(embeddedArchiveFile, nupkgPath));
